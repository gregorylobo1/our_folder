#!/usr/bin/env python
'''
leg finding script - version 3
last editted on 18/09/2019
by Phillip Luu
'''
import rospy
...
def conf_det(pre_x, pre_y, new_x, new_y):
    ...
    stride_est = 0.7
    conf_factor = 0.5
    x_err = abs(pre_x - new_x)
    y_err = abs(pre_y - new_y)
    if x_err < stride_est and y_err < stride_est:
        conf = 1
    else:
        print('x_diff = ' + str(x_err) + ' and y_diff = ' + str(y_err))
        conf = math.sqrt(x_err*x_err + y_err*y_err)
    return conf
...
def listener():
    rospy.Subscriber("/new_scan", LaserScan, read_cb, queue_size = 1)
